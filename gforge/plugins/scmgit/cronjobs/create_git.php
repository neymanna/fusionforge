 ...
 * create_git.php
 *
 * Original Author: Ferenc Székely <ferenc@maemo.org>
 *
 ...
 * from the FusionForge server thru ssh. naturally these two can be
...
$create_repo_script = dirname(dirname(__FILE__)).'/bin/create_git_repo.sh';

// Detect if repository is remote or local to the server.
$repo_is_remote = ($default_git_server !== $GLOBALS['sys_default_domain']);
...
$res = db_query("SELECT is_public, enable_anonscm, unix_group_name, groups.group_id
	...
	AND plugins.plugin_name='scmgit'");
...
while ( $row =& db_fetch_array($res) ) {
	...
	$git_clone_cmd .= (($default_git_use_dav == 'true') ? 'http' : 'git+ssh');
	$git_clone_cmd .= (($default_git_use_ssl == 'true') ? 's' : '') . "://";
	$git_clone_cmd .= $default_git_server . "/";
	$git_clone_cmd .= (($default_git_root) ? $default_git_root ."/" : '');
	...
	$err .= " at " .  $default_git_server . ".\n";

	print "DEBUG: $create_cmd\n";
	...
	$err .= $git_clone_cmd . "\n";
	$err .= (($default_git_use_ssl == 'true') ? $ssl_cert_warning : '');
}
...
/** function check_svn_mail($project, $repos) { $contents =
@file_get_contents($repos."/hooks/post-commit"); if ( strstr($contents,
"svncommitemail") == FALSE ) {
add_svn_mail_to_repository($project,$repos); } } function
add_svn_mail_to_repository($unix_group_name,$repos) { global
$sys_lists_host,$file_owner,$sys_plugins_path; if
(file_exists($repos.'/hooks/post-commit')) { $FOut =
fopen($repos.'/hooks/post-commit', "a+"); } else { $FOut =
fopen($repos.'/hooks/post-commit', "w"); $Line = '#!/bin/sh'."\n"; //
add this line to first line or else the script fails } if($FOut) { $Line
.= ' #begin added by svncommitemail
'.$sys_plugins_path.'/svncommitemail/bin/commit-email.pl '.$repos.' "$2"
'.$unix_group_name.'-commits@'.$sys_lists_host.' #end added by
svncommitemail'; fwrite($FOut,$Line); `chmod +x
$repos'/hooks/post-commit'`; `chmod 700 $repos'/hooks/post-commit'`;
`chown $file_owner $repos'/hooks/post-commit'`; fclose($FOut); } }

function check_svn_www($project, $repos) { $contents =
@file_get_contents($repos."/hooks/post-commit"); if ( strstr($contents,
"svnwww") == FALSE ) { add_svn_www_to_repository($project,$repos); } }

function add_svn_www_to_repository($unix_group_name,$repos) { global
$sys_lists_host,$file_owner,$sys_plugins_path; if
(file_exists($repos.'/hooks/post-commit')) { $FOut =
fopen($repos.'/hooks/post-commit', "a+"); } else { $FOut =
fopen($repos.'/hooks/post-commit', "w"); $Line = '#!/bin/sh'."\n"; //
add this line to first line or else the script fails } if($FOut) { $Line
.= "\n\n".'#svnwww'."\n"; $Line .= 'svn ls file://'.$repos.'/www >
/dev/null 2>&1'."\n"; $Line .= "if [ \$? -eq \"0\" ]; then\n"; $Line .=
' rm -rf /var/www/homedirs/groups/'.$unix_group_name.'/.previous'."\n";
$Line .= ' mv /var/www/homedirs/groups/'.$unix_group_name.'
/var/www/homedirs/groups/'.$unix_group_name.'.previous'."\n"; $Line .= '
svn export --force file://'.$repos.'/www
/var/www/homedirs/groups/'.$unix_group_name."\n"; $Line .= ' chown -R
www-data:www-data /var/www/homedirs/groups/'.$unix_group_name."\n";
$Line .= ' mv /var/www/homedirs/groups/'.$unix_group_name.'.previous
/var/www/homedirs/groups/'.$unix_group_name.'/.previous'."\n"; $Line .=
'fi'."\n"; $Line .= '#end_of_svnwww'."\n"; fwrite($FOut,$Line); `chmod
+x $repos'/hooks/post-commit'`; `chmod 700 $repos'/hooks/post-commit'`;
`chown $file_owner $repos'/hooks/post-commit'`; fclose($FOut); } } */
